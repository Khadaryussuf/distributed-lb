...
def set_replica_count(target_n):
    ...
    if target_n > current_n:
        ...
        requests.post(f"{LOAD_BALANCER_URL}/add", json={"n": diff}, timeout=30)
    elif target_n < current_n:
        diff = current_n - target_n
        requests.delete(f"{LOAD_BALANCER_URL}/rm", json={"n": diff}, timeout=30)
...
if __name__ == "__main__":
    ...
    plt.plot(N_VALUES, average_loads, marker="o", color="seagreen")
    plt.xlabel("Number of Servers (N)")
    plt.ylabel("Average Load per Server (requests)")
    plt.title(f"A-2 (Improved Hash): Average Load vs Number of Servers")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("a2_line_chart_improved.png")
    print("\nSaved chart to a2_line_chart_improved.png")